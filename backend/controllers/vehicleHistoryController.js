...
const isAuthorized = (user, vehicle) => {
    return user.role === 'admin' || 
           (user.role === 'driver' && vehicle.driver.includes(user._id));
};

const vehicleHistoryController = {
    // Add new vehicle history entry
    addVehicleHistory: async (req, res) => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: 'Error creating vehicle history',
                error: error.message
            });
        }
    },
    ...
    updateVehicleHistory: async (req, res) => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: 'Error updating vehicle history',
                error: error.message
            });
        }
    }
...
module.exports = vehicleHistoryController;